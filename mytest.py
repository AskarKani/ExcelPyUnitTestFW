...
class Mytest:
    ...
    def split_check_path(self, header, paths, is_folder=False):
        # Check for brackets "[ ]"
        if paths.find("[") == -1 or paths.find("]") == -1:
            sys.exit(f"!! EXITING !! {header} value is missing \"[\" or \"]\" ")

        # Check for empty entries
        paths_check_empty = paths.strip("[]").strip(" ")
        ...
        for i in range(0, len(paths)):
            ...
            if not is_folder:
                if not os.path.isfile(paths[i]):
                    sys.exit(f"!! EXITING !! {header}:{paths[i]} path is incorrect...")
            else:
                if not os.path.isdir(paths[i]):
                    sys.exit(f"!! EXITING !! {header}:{paths[i]} folder path is incorrect...")
        return paths
    ...
    def read_excel(self):
        ...
        self.lib_name_s = self.split_check_path("$LIB_NAME(s)", self.sheet['B2'].value)
        ...
        self.flags_gcc = self.split_check_path("$FLAGS_GCC", self.sheet['B6'].value)
    ...
    def compile(self):
        ...
        for file in self.func_file_s:
            ...
            self.terminal_exec(f"g++ -fpic -c {file} {include_path_gcc} {lib_include_paths} -o {out_cpp_path} ")
            out_file.append(out_cpp_path)

        lib_name = "libtest.so"
        lib_out_path = self.unit_test_out / lib_name
        self.terminal_exec(f"g++ -shared -o {lib_out_path} {self.append_option('', out_file)}")
